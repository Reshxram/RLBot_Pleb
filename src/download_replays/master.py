...
while True:
    try:
        ...
        if f'{status["Day"]:02d}-{status["Month"]:02d}-{status["Year"]}' == datetime.today().strftime('%d-%m-%Y'):
            print("Done :)")
            exit(0)
        ...
        print("Probably internet down, waiting 10min")
        for x in tqdm(range(600)):
            time.sleep(1)
